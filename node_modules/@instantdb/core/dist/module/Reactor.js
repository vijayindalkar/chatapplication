...
export default class Reactor {
    constructor(config, Storage = IndexedDBStorage, NetworkListener = WindowNetworkListener, versions) {
        ...
        this._wsOnOpen = (e) => {
            ...
            if (this._ws !== targetWs) {
                log.info("[socket][open]", targetWs._id, "skip; this is no longer the current ws");
                return;
            }
        ...
        this._wsOnMessage = (e) => {
            ...
            if (this._ws !== targetWs) {
                log.info("[socket][message]", targetWs._id, m, "skip; this is no longer the current ws");
                return;
            }
        ...
        this._wsOnError = (e) => {
            const targetWs = e.target;
            if (this._ws !== targetWs) {
                log.info("[socket][error]", targetWs._id, "skip; this is no longer the current ws");
                return;
            }
        ...
        this._wsOnClose = (e) => {
            const targetWs = e.target;
            if (this._ws !== targetWs) {
                log.info("[socket][close]", targetWs._id, "skip; this is no longer the current ws");
                return;
            }
            ...
            if (this._isShutdown) {
                log.info("[socket][close]", targetWs._id, "Reactor has been shut down and will not reconnect");
                return;
            }
            log.info("[socket][close]", targetWs._id, "schedule reconnect, ms =", this._reconnectTimeoutMs);
            setTimeout(() => {
                this._reconnectTimeoutMs = Math.min(this._reconnectTimeoutMs + 1000, 10000);
                if (!this._isOnline) {
                    log.info("[socket][close]", targetWs._id, "we are offline, no need to start socket");
                    return;
                }
                this._startSocket();
    ...
    _startSocket() {
        if (this._ws && this._ws.readyState == WS_CONNECTING_STATUS) {
            ...
            // effectively fresh.
            log.info("[socket][start]", this._ws._id, "maintained as current ws, we were still in a connecting state");
            return;
        }
