import type { EntitiesDef, IContainEntitiesAndLinks, InstantGraph, LinkAttrDef, ResolveAttrs, ResolveEntityAttrs } from "./schemaTypes";
type BuiltIn = Date | Function | Error | RegExp;
type Expand<T> = T extends BuiltIn ? T : T extends object ? T extends infer O ? {
...
type QueryResponse<Q, Schema, WithCardinalityInference extends boolean = false> = Schema extends InstantGraph<infer E, any> ? InstaQLQueryResult<E, Q, WithCardinalityInference> : ResponseOf<{
    [K in keyof Q]: Remove$<Q[K]>;
}, Schema>;
type InstaQLResponse<Schema, Q> = Schema extends IContainEntitiesAndLinks<any, any> ? InstaQLResult<Schema, Q> : never;
...
type InstaQLEntitySubqueryResult<Schema extends IContainEntitiesAndLinks<EntitiesDef, any>, EntityName extends keyof Schema["entities"], Query extends InstaQLEntitySubquery<Schema, EntityName> = {}> = {
    [QueryPropName in keyof Query]: Schema["entities"][EntityName]["links"][QueryPropName] extends LinkAttrDef<infer Cardinality, infer LinkedEntityName> ? LinkedEntityName extends keyof Schema["entities"] ? Cardinality extends "one" ? InstaQLEntity<Schema, LinkedEntityName, Query[QueryPropName]> | undefined : InstaQLEntity<Schema, LinkedEntityName, Query[QueryPropName]>[] : never : never;
};
type InstaQLQueryEntityLinksResult<Entities extends EntitiesDef, EntityName extends keyof Entities, Query extends {
...
type InstaQLEntity<Schema extends IContainEntitiesAndLinks<EntitiesDef, any>, EntityName extends keyof Schema["entities"], Subquery extends InstaQLEntitySubquery<Schema, EntityName> = {}> = Expand<{
    id: string;
} & ResolveEntityAttrs<Schema["entities"][EntityName]> & InstaQLEntitySubqueryResult<Schema, EntityName, Subquery>>;
type InstaQLQueryEntityResult<Entities extends EntitiesDef, EntityName extends keyof Entities, Query extends {
    ...
    id: string;
} & ResolveAttrs<Entities, EntityName> & InstaQLQueryEntityLinksResult<Entities, EntityName, Query, WithCardinalityInference>;
type InstaQLQueryResult<Entities extends EntitiesDef, Query, WithCardinalityInference extends boolean> = {
...
type InstaQLEntitySubquery<Schema extends IContainEntitiesAndLinks<EntitiesDef, any>, EntityName extends keyof Schema["entities"]> = {
    [QueryPropName in keyof Schema["entities"][EntityName]["links"]]?: InstaQLEntitySubquery<Schema, Schema["entities"][EntityName]["links"][QueryPropName]["entityName"]>;
};
type InstaQLQuerySubqueryParams<S extends IContainEntitiesAndLinks<any, any>, E extends keyof S["entities"]> = {
...
export { Query, QueryResponse, InstaQLResponse, PageInfoResponse, InstantObject, Exactly, Remove$, InstaQLQueryResult, InstaQLParams, InstaQLQueryEntityResult, InstaQLEntity, InstaQLResult, Cursor, InstaQLQueryParams, };
